...
import { testimonialCreateSchema } from "@/lib/validators/content.validator";
...
export const GET = asyncHandler(async (request: NextRequest) => {
  ...
  const { items, meta } = await listDocuments(Testimonial, {
    page,
    limit,
    skip,
    search: q.search,
  ...
  return successResponse(mapWithIds(items), "Testimonials fetched", meta);
});
...
export const POST = asyncHandler(async (request: NextRequest) => {
  ...
  const doc = await createDocument(Testimonial, parsed.data);
  return successResponse(withId(doc), "Testimonial created", undefined, 201);
});
